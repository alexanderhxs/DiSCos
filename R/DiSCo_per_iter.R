...
DiSCo_per_iter <- function(c_df, c_df.q, t_df, T0, peridx, evgrid, idx, grid_df, M=1000,
    ...
    for (i in 1:length(perc)){
      perc[[i]][[1]]=t_df[[i]]
      #ADDED, BUG IN ORIGINAL PAPER: we need to update the quantiles for the permuted target as well, otherwise the distance will be wrong in the mixture case
      perc.q[[i]][,1]=myQuant(t_df[[i]], evgrid, qmethod, qtype=qtype)
    }
