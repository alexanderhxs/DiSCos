...
DiSCo <- function(df, id_col.target, t0, M = 1000, G = 1000, num.cores = 1, permutation = FALSE, q_min = 0, q_max = 1,
  ...
  for (i in seq_along(periods)) {
    ...
    message(sprintf("Berechne Periode %d von %d (Jahr: %s)...", i, total_periods, as.character(yy)))
    results.periods[[i]] <- DiSCo_iter(
      yy = yy,
      df = df,
      evgrid = evgrid,
      id_col.target = id_col.target,
      M = M,
      G = G,
      T0 = T0,
      qmethod = qmethod,
    ...
    message(sprintf(" -> Periode %d abgeschlossen.", i))
  }
  ...
  if (CI) {
    ...
    for (b in 1:boots) {
      if (b %% 5 == 0){
      message(sprintf("Berechne Bootstrap-Sample %d von %d...", b, boots))
      }
      CI_bootmat[[b]] <- DiSCo_CI(
        b,
        controls = controls,
        target = target,
        T_max = T_max,
    ...
    message("Alle Bootstrap-Samples erfolgreich berechnet!")
