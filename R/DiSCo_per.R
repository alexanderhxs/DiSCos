...
DiSCo_per <- function(results.periods, T0, ww=0, peridx=0, evgrid=seq(from=0, to=1, length.out=101),
  ...
  total_perms <- length(peridx)

  for (idx in seq_len(total_perms)) {

    message(sprintf("Berechne Placebo-Permutation %d von %d...", idx, total_perms))
    distp[[idx]] <- DiSCo_per_iter(
      c_df = c_df,
      c_df.q = controls.q,
      t_df = t_df,
      T0 = T0,
  ...
  message("Alle Placebo-Permutationen erfolgreich berechnet!")
